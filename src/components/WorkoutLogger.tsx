...
import { Plus, Trash2, Save, Dumbbell, Clock, Flame, ChevronRight, ChevronLeft } from 'lucide-react';
import { format, getDay, addDays, startOfWeek } from 'date-fns';
import { collection, addDoc, query, where, getDocs, doc, writeBatch, deleteDoc, updateDoc } from 'firebase/firestore';
                ...
                <div key={exIdx} className="bg-[#E4E3E0]/30 p-4 md:p-6 border border-[#141414]/10">
                  <div className="flex justify-between items-center mb-4">
                    <input 
                      ...
                      className="bg-transparent border-b border-[#141414] py-1 font-bold uppercase tracking-tight focus:outline-none w-full mr-4"
                    />
                    <button 
                      onClick={() => {
                        const updated = [...newWorkout.exercises];
            ...
            {newWorkout.name !== 'Treadmill Cardio' && (
              <button 
                onClick={() => {
                  setNewWorkout({
                    ...newWorkout,
                    exercises: [...newWorkout.exercises, { name: '', sets: [{ reps: 10, weight: 20 }] }]
                  });
                }}
